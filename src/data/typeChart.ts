/**
 * TABELA DE EFETIVIDADE DE TIPOS (18 tipos modernos).
 *
 * Multiplicadores:
 *   2   = super efetivo
 *   1   = dano normal
 ...
 *   0   = imune
 *
 * Formato: TYPE_CHART[tipoAtaque][tipoDefensor] = multiplicador.
 *
 * A matriz é explícita (todas as 18×18 combinações preenchidas) para que
 * nenhuma combinação dependa de um valor "padrão" implícito — evita bugs
 * silenciosos por entradas esquecidas. Acesso é sempre data-driven
 * (typeChart[attackType][defenseType]); nenhuma regra de tipo é escrita
 * dentro do motor de cálculo (ver core/typeCalculator.ts).
 *
 * Regra própria do RPG — não é uma reprodução integral do cálculo interno
 * dos jogos Pokémon.
 */

import type { PokemonType } from "../core/models";

/** Um multiplicador de efetividade válido. */
export type Effectiveness = 0 | 0.5 | 1 | 2;

/** Matriz completa de efetividade. */
export type TypeChart = Record<PokemonType, Record<PokemonType, Effectiveness>>;
